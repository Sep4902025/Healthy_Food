...
exports.createContactUs = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ status: "error", error: "Thiếu thông tin cần thiết" });
    }
...
exports.hardDeleteContactUs = async (req, res) => {
  ...
  try {
    ...
    if (!contact) {
      return res
        .status(404)
        .json({ success: false, message: "Contact không tồn tại!" });
    }
    await ContactUs.findByIdAndDelete(id);
    res
      .status(200)
      .json({ success: true, message: "Contact Us đã bị xóa vĩnh viễn" });
  } catch (error) {
    console.error("Lỗi xóa liên hệ:", error);
    res
      .status(500)
      .json({ success: false, message: "Lỗi xóa cứng Contact Us" });
  }
};
