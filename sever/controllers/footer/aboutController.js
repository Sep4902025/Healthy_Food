...
exports.createAboutUs = async (req, res) => {
  try {
    ...
    if (!req.body || Object.keys(req.body).length === 0) {
      return res
        .status(400)
        .json({ status: "fail", error: "Body request rỗng!" });
    }
    ...
    if (!bannerUrl || !content) {
      return res
        .status(400)
        .json({ status: "fail", error: "Thiếu dữ liệu đầu vào!" });
    }
    ...
    res
      .status(500)
      .json({ status: "fail", error: "Lỗi server khi tạo About Us" });
  }
};
...
exports.hardDeleteAboutUs = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: "success", message: "About Us đã bị xóa vĩnh viễn" });
  } catch (error) {
