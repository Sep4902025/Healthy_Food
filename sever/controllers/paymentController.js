...
exports.createPaymentUrl = catchAsync(async (req, res) => {
  const { userId, mealPlanId, amount, clientType } = req.body; // Lấy clientType từ body
  const clientIp =
  ...
  const { paymentUrl, paymentId } = await paymentService.createPaymentUrl(
    ...
    amount,
    clientIp,
    clientType // Truyền clientType vào service
  );
...
exports.vnpayReturn = catchAsync(async (req, res) => {
  ...
  const redirectUrl = await paymentService.vnpayReturn(vnp_Params);
