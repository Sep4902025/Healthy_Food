...
exports.deleteDish = async (req, res) => {
  try {
    ...
    if (!deletedDish) return res.status(404).json({ status: "fail", message: "Dish not found" });
    res.status(200).json({ status: "success", message: "Dish marked as deleted" });
  } catch (error) {
    res.status(500).json({ status: "fail", message: error.message });
  }
};
...
exports.updateIngredient = async (req, res) => {
  try {
    const updatedIngredient = await Ingredients.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
...
exports.deleteIngredient = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: "success", message: "Ingredient marked as deleted" });
  } catch (error) {
    res.status(500).json({ status: "fail", message: error.message });
  }
};
