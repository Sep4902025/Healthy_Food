...
const TableIngredient = () => {
  ...
  const handleDelete = async (id) => {
    const isConfirmed = window.confirm("Bạn có chắc chắn muốn xóa nguyên liệu này không?");
  ...
  const handleToggleVisibility = async (ingredient) => {
    ...
    const response = await ingredientService.updateIngredient(ingredient._id, updatedIngredient);
    if (response.success) {
      fetchIngredients();
    } else {
    ...
    <div>
      ...
      {showEditForm && (
        <div className="bg-white p-4 border border-gray-300 mb-4">
          ...
          {["name", "description", "type", "season", "calories", "protein", "carbs", "fat", "unit", "nutritionalInfo"].map((field) => (
