      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mealPlans.length === 0 ? (
          <div className="col-span-full bg-gray-50 p-8 text-center rounded-lg border border-dashed border-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-12 h-12 mx-auto text-gray-400 mb-3">
          ...
          mealPlans.map((mealPlan) => (
            <div key={mealPlan._id} className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-100 flex flex-col h-full hover:shadow-lg transition duration-200">
              <div className="p-5 flex-grow">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-xl font-semibold text-gray-800">{mealPlan.title}</h3>
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${mealPlan.type === 'fixed' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
                    {mealPlan.type}
                  </span>
                </div>
              ...
              <div className="bg-gray-50 p-4 border-t flex justify-between items-center">
                <Link 
                  to={`/meal-plan/${mealPlan._id}/meal-days`}
